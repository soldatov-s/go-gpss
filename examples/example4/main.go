...
func main() {
	...
	visitors_g := gpss.NewGenerator("Visitors", 10, 5, 0, 0, nil)
	// visitors_g := NewGenerator("Visitors", 0, 0, 0, 1, nil)
	out := gpss.NewHole("Out")
	visitors_q := gpss.NewQueue("Visitors queue")
	...
	hostes1_f := gpss.NewFacility("hostess 1", 5, 3)
	hostes2_f := gpss.NewFacility("Hostess 2", 5, 3)
	// 4. Create are Tables
	cnt_tables := 24
	tables_in := make([]gpss.IBaseObj, cnt_tables)
	tables_out := make([]gpss.IBaseObj, cnt_tables)
	for i := 0; i < cnt_tables; i++ {
		table_name := fmt.Sprintf("Table %d", i+1)
		tables_in[i], tables_out[i] = gpss.NewBifacility(table_name)
	}
	...
	check_empty_table := gpss.NewCheck("Check empty table", CheckEmptyTableHndl, nil)
	// 6. Create the Queues and Facilities for waiters
	cnt_waiters := 8
	waiters_queue := make([]gpss.IBaseObj, cnt_waiters)
	waiters_facility := make([]gpss.IBaseObj, cnt_waiters)
	for i := 0; i < cnt_waiters; i++ {
		queue_name := fmt.Sprintf("Queue waiter %d", i+1)
		waiters_queue[i] = gpss.NewQueue(queue_name)
		facility_name := fmt.Sprintf("Waiter %d", i+1)
		waiters_facility[i] = gpss.NewFacility(facility_name, 5, 3)

	}
	// 7. Create the Split for splitting Visitors order to dishes
	// Maybe 1 or 5 dishes, includes bar
	dishes_sp := gpss.NewSplit("Selected dishes", 3, 2, nil)
	// 8. Create the Check that transact is a coocked dishes. If false, this transact is
	// an order from tables, needs split to dishes
	dish_state := gpss.Parameter{Name: "State", Value: "After kitchen"}
	check_is_cooked := gpss.NewCheck("After kitchen?", nil, dishes_sp, dish_state)
	// 9. Create the Queue and Facility for cooks and barmans. Each cook cooking
	// only one type of dishes: meat, sushi, salats, dessert
	cook1_q := gpss.NewQueue("Queue of orders to cook 1 (meat dishes)")
	cook2_q := gpss.NewQueue("Queue of orders to cook 2 (fish dishes)")
	cook3_q := gpss.NewQueue("Queue of orders to cook 3 (salats)")
	cook4_q := gpss.NewQueue("Queue of orders to cook 4 (dessert)")
	bar_q := gpss.NewQueue("Queue of orders to bar")
	cook1_f := gpss.NewFacility("Cook 1 (meat dishes)", 15, 5)
	cook2_f := gpss.NewFacility("Cook 2 (sushi)", 7, 3)
	cook3_f := gpss.NewFacility("Cook 3 (salats)", 10, 4)
	cook4_f := gpss.NewFacility("Cook 4 (dessert)", 20, 5)
	barman1_f := gpss.NewFacility("Barman 1", 4, 2)
	barman2_f := gpss.NewFacility("Barman 2", 4, 2)
	// 10. Create the Assign that dish is cooked, includes bar
	assign := gpss.NewAssign("After kitchen", dish_state)
	...
	check_tb_number := func(id_table int) gpss.HandleCheckingFunc {
	...
	check_tb := make([]gpss.IBaseObj, cnt_waiters)
	for i := 0; i < cnt_waiters; i++ {
		check_name := fmt.Sprintf("Is it order for table %d, %d, %d?", i*3+1, i*3+2, i*3+3)
		check_tb[i] = gpss.NewCheck(check_name, check_tb_number(i*3+1), nil)
	}
